...
--			m.visuald.projectConfigurations,
			...
			-- HAX: everything! split this into bits...
			m.visuald.everything
		}
	end

	function m.visuald.obj(cfg)
		_p(2,'<obj>0</obj>')
	end

	function m.visuald.link(cfg)
		_p(2,'<link>0</link>')
	end

	function m.visuald.everything(cfg)
		...
		_p(2,'<singleFileCompilation>0</singleFileCompilation>', iif(cfg.flags.SeparateCompilation, '2', '0'))
...
--			_p(2,'<runCv2pdb>%s</runCv2pdb>', iif(cfg.flags.Symbols, '1', '0'))
	...
	function m.visuald.projectConfigurations(prj)
		...
		for cfg in project.eachconfig(prj) do
			...
			p.callArray(m.elements.config, cfg)
